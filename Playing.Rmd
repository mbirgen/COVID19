...
```{r, echo=FALSE}
pBremer <-qplot(date, Bremer.SS, data = clean[-(1:71),], geom = c("point", "smooth")) +
  ggtitle("Known Active COVID-19 Cases in Bremer County")
pButler <-qplot(date, Butler.SS, data = clean[-(1:116),], geom = c("point", "smooth")) +
  ggtitle("Known Active COVID-19 Cases in Butler County")
png("pButler.png")
pButler
dev.off()
pButler
pBlackHawk <-qplot(date, BlackHawk.SS, data = clean[-(1:116),], geom = c("point", "smooth")) +
