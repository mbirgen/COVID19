...
percent_pos7 <- round(clean[nrow(clean)-4,16]*100, digits = 1)
...
Today we had `r new_pos` new positive tests which is `r percent_pos`% (`r percent_pos7`% in the last week) of the tests recorded in the last 24 hours. There are `r sick` people actively sick and `r percent_hospital` (`r hospital`) are hospitalized. `r percent_ICU`% (`r ICU`) of those hospitalized are in the ICU. 
...
pBremer <-qplot(date, Bremer.SS/Bremer_Population*100000, data = clean[-(1:71),], geom = c("point", "smooth")) +
  ggtitle("Known Active COVID-19 Cases in Bremer County Per 100,000 People")
pButler <-qplot(date, Butler.SS/Butler_Population*100000, data = clean[-(1:116),], geom = c("point", "smooth")) +
  ggtitle("Known Active COVID-19 Cases in Butler County Per 100,000 People")
png("pButler.png")
pButler
dev.off()
pButler
pBlackHawk <-qplot(date, BlackHawk.SS/BlackHawk_Population*100000, data = clean[-(1:116),], geom = c("point", "smooth")) +
