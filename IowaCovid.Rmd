...
covid19[covid19$date == i,
 ...
 covid19[covid19$date == as.Date(date),"Recovered"] =
     subset(clean, date == (day-28))$positive - subset(clean, date == (day-28))$deaths
...
`r clean$New.Deaths[nrow(clean)]` deaths in the last week. One in `r round(IowaPop/clean[nrow(clean),]$deaths, digits = 0)` people in Iowa has died of COVID-19. There are `r covid19$hospitalized[nrow(covid19)]` (`r round(tail(clean$hospitalized, n=1)/tail(Active,n=1)*100,1)`% of active) Iowans hospitalized for COVID-19. Of those, `r clean$ICU[nrow(clean)]` (`r tail( round(clean$ICU/clean$hospitalized*100,1) , n=1)`% of hospitalized) are in the ICU.

One year ago, we had `r format(subset(clean, date == (day-365))$Still.Sick, scientific = FALSE, big.mark=",")` actively sick and `r subset(clean, date == (day-365))$New.Positive` new positive patients per day in Iowa. Two years ago, we had `r format(subset(clean, date == (day-730))$Still.Sick, scientific = FALSE, big.mark=",")` actively sick and `r subset(clean, date == (day-730))$New.Positive` new positive patients per day in Iowa.
