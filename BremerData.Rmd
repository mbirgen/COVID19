 ...
 ***
 `r as.integer(Butlerlastrow["Active"])` active cases`r if(Butemp$Active) {" (record)"}` in Butler County, IA today with `r as.integer(Butlerlastrow["Hospitalized"])` hospitalized`r if(Butemp$Hospitalized) {" (record)"}`. If you are in a group of `r groupsize` people in Butler County there is a `r round(Risk$Butler.Risk*100, digits = 2)`% chance someone in the group is positive.
